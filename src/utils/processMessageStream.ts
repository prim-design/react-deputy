...
export async function processMessageStream(
  ...
  while (!done) {
    ...
    for (let i = 0; i < strServerEvents.length; i++) {
      ...
      // Skip empty strings
      if (strServerEvent.trim() === "" || !strServerEvent) {
        continue;
      }
      strServerEvent = jsonAutoComplete(strServerEvent);

      try {
        const serverEvent: OpenAI.Beta.AssistantStreamEvent =
          JSON.parse(strServerEvent);
        ...
        switch (event) {
          ...
          case "thread.message.created":
            ...
            setMessages((prevMessages) => [
              data,
              ...prevMessages.filter(
                (message) => message.role !== "assistantPlaceholder"
              ),
            ]);
            break;
                ...
                actions.forEach((action) => {
                  ...
                  if (
                    ...
                    functionSnapshots[data.id] = {
                      ...functionSnapshots[data.id],
                      snapshot:
                        functionSnapshots[data.id].snapshot +
                        action.function.arguments,
                    };
                  }
                });
          ...
          case "thread.run.requires_action":
            ...
            if (data.required_action?.submit_tool_outputs) {
              ...
              const tool_outputs = await config.tools?.processAssistantActions(
                actions
              );
              ...
              const response = await config.toolOutputsApi?.({
                ...
                tool_outputs: tool_outputs
                  ? refineToolOutputs(tool_outputs)
                  : [],
              });
