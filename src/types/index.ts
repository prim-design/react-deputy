...
export type FunctionDeltas = Record<string, { functionName: string; snapshot: string }>
...
export type Message = OpenAI.Beta.Threads.Message | PlaceholderMessage | UserMessage

interface AssistantWithThread {
  thread_id: string
  onThreadCreated?: (data: OpenAI.Beta.Threads.Thread) => void | Promise<void>
  createThreadApi?(data: { assistant_id?: string }): Promise<OpenAI.Beta.Threads.Thread>
}
...
type AssistantConfigBase = {
  ...
  onMessageInProgress?: (data: OpenAI.Beta.Threads.Messages.Message) => void | Promise<void>
  onMessageCompleted?: (data: OpenAI.Beta.Threads.Messages.Message) => void | Promise<void>
  onRunQueued?: (data: OpenAI.Beta.Threads.Runs.Run) => void | Promise<void>
  onRunInProgress?: (data: OpenAI.Beta.Threads.Runs.Run) => void | Promise<void>
  onRunStepCreated?: (data: OpenAI.Beta.Threads.Runs.RunStep) => void | Promise<void>
  onRunStepInProgress?: (data: OpenAI.Beta.Threads.Runs.RunStep) => void | Promise<void>
  onRunStepDelta?: (data: OpenAI.Beta.Threads.Runs.Steps.RunStepDeltaEvent) => void | Promise<void>
  onRunStepCompleted?: (data: OpenAI.Beta.Threads.Runs.RunStep) => void | Promise<void>
  onRunStepRequiredAction?: (data: OpenAI.Beta.Threads.Runs.Run) => void | Promise<void>
...
export type AssistantConfig = Assistant & (AssistantConfigWithTools | AssistantConfigWithoutTools)
