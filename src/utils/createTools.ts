...
interface Steps<T = void, Omitted extends string = never> {
  ...
  ): Omit<Steps<T, Omitted | "onComplete">, "onComplete" | "input" | Omitted> &
    InternalTool;

  onDelta(
    func: (input: Partial<T extends void ? never : T>) => unknown
  ): Omit<Steps<T, Omitted | "onDelta">, "onDelta" | "input" | Omitted> &
    InternalTool;
  /**
...
> & {
  ...
  const combinedCustomTools = {
    ...
    async processAssistantActions(
      ...
      return (
        await Promise.all(
          customTools.map((t) => t.processAssistantActions(data))
        )
      ).flat();
    },
    processActionDeltas(data: FunctionDeltas) {
