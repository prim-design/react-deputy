  ...
  switch (action.type) {
    ...
    case 'REMOVE_NODE':
      try {
        ...
        console.error(`Error while removing node with id ${action.id}: ${error}`)
        return state
      }
    default:
      return state
...
const useTree = (): UseTreeReturn => {
  ...
  const printTree = useCallback(
    (categories: string[]): string => {
      ...
      const filteredTree = filterTreeByCategories(tree, categoriesSet)

      let output = ''
      filteredTree.forEach((node, index) => {
...
function filterTreeByCategories(tree: Tree, categories: Set<string>): Tree {
  return tree
    .filter((node) => setsHaveIntersection(node.categories, categories))
    .map((node) => ({
      ...node,
      children: filterTreeByCategories(node.children, categories),
    }))
}
